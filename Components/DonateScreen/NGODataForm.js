...
const NGOForm = ({ navigation }) => {
  ...
  const [userInterest, setUserInterest] = useState('Food');


  let  x = ngoName && numMembers && locationData && numPeopleServed && userInterest
  useEffect(() => {
  ...
  const handleNGOReg = async () => {
    ...
    if (userDoc.exists()) {
      Alert.alert("NGO Already exists", "Document with this ID already exists.");
    } else {
      ...
      Alert.alert("Data Successfully Stored in Firebase/Firestore");
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.MinCont}>
        ...
        <Text style={styles.Name}>Interest in offer:</Text>
        <View style={styles.inputView}>
